...
namespace ProductComparison\Model\Tools;
...
trait PositionManagementTrait
{
    ...
    protected function movePositionUpOrDown($up = true)
    {
        ...
        if ($up === true) {
            ...
            $search->filterByPosition(array('min' => $my_position+1))->orderByPosition(Criteria::ASC)->filterByTemplateId($template_id);
        }
    ...
    public function changeAbsolutePosition($newPosition)
    {
        ...
        if ($newPosition != null && $newPosition > 0 && $newPosition != $current_position) {
            ...
            if ($newPosition > $current_position) {
                ...
                $search->filterByPosition(array('min' => 1+$current_position, 'max' => $newPosition))->filterByTemplateId($template_id);

                $delta = -1;
            } else {
                // The new position is brefore the current position -> we will offset - 1 all categories located between us and the new position
                $search->filterByPosition(array('min' => $newPosition, 'max' => $current_position - 1))->filterByTemplateId($template_id);
